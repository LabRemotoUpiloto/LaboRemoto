...
export default function UserMessageBubble({ msg, isSending, onDelete, onSaveEdit, onEditStateChange }: UserMessageBubbleProps) {
  ...
  if (isEditing) {
    return (
      <div className="flex flex-col items-end w-full animate-in fade-in">
        <div className="w-full max-w-[85%] bg-[var(--background-tertiary)] border border-[var(--border-subtle)] rounded-xl p-3 flex flex-col gap-2">
          <Textarea
            value={draft}
            ...
            autosize
            variant="unstyled"
            styles={{ input: { color: 'var(--text-primary)', fontSize: 13, lineHeight: 1.5, padding: 0 } }}
            className="w-full"
          />
          <div className="flex items-center justify-between mt-1">
            <span className="text-[10px] text-[var(--text-muted)] hidden sm:inline-block">Enter · enviar &nbsp;·&nbsp; Esc · cancelar</span>
            <Group gap="xs" justify="flex-end" className="ml-auto">
              <Button size="compact-xs" variant="subtle" color="gray" onClick={cancelEdit} className="text-[var(--text-secondary)] hover:text-[var(--text-primary)]">Cancelar</Button>
  ...
  return (
    <div className="flex flex-col items-end w-full group/user">
      <div className="chat-bubble chat-bubble-user rounded-2xl rounded-tr-sm px-3.5 py-2.5 max-w-[85%] shadow-sm relative break-words text-[13px] leading-relaxed">
        <div className="flex flex-col gap-1.5">
          ...
          {msg.meta?.attachedFileName && (
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[var(--background-primary)] border border-[var(--border-strong)] text-[11px] max-w-full self-start text-[var(--text-secondary)]">
      ...
      <div className="flex items-center gap-0.5 mt-1 opacity-0 group-hover/user:opacity-100 transition-opacity">
        <ActionIcon
          ...
          disabled={isSending}
          onClick={startEdit}
          className="text-[var(--text-secondary)]/60 hover:text-[var(--text-primary)] hover:bg-[var(--interactive-hover)]"
        >
          <Pencil size={12} />
        </ActionIcon>
        <ActionIcon
          ...
          className="text-[var(--text-secondary)]/60 hover:text-[var(--danger-text)] hover:bg-[var(--danger-bg)]"
        >
          <Trash2 size={12} />
        </ActionIcon>
      </div>
