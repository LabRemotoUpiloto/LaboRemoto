...
// Perf: igual que ssh_core::exec::acquire_ssh2 — el handshake+auth nunca se
// hace con el lock de `SESSIONS` (global, compartido por todas las
// sesiones) retenido, para no bloquear terminal/SFTP/otros GPIO mientras
// esta sesión se conecta.
fn get_or_connect_cached(id: &str) -> Result<Arc<Mutex<CachedSsh2>>, CommandError> {
  let (host, port, user, password) = {
    let map = SESSIONS.lock().map_err(|e| CommandError::internal("LOCK_POISONED", format!("SESSIONS lock poisoned: {}", e)))?;
...
pub async fn rpi_pins_status(id: String) -> Result<Vec<RpiGpioLine>, CommandError> {
  tokio::task::spawn_blocking(move || -> Result<Vec<RpiGpioLine>, CommandError> {
...
pub async fn rpi_pin_read(id: String, gpio: u32) -> Result<RpiGpioLine, CommandError> {
  tokio::task::spawn_blocking(move || -> Result<RpiGpioLine, CommandError> {
    let arc_cached = get_or_connect_cached(&id)?;
