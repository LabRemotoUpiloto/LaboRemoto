...
\subsection{Arquitectura General: Topología de Red y Subsistemas Lógicos}
...
A nivel interno, la aplicación cliente se divide en tres subsistemas lógicos fundamentales comunicadas asíncronamente mediante un canal de paso de mensajes (IPC, de sus siglas en inglés), lo cual garantiza alta resiliencia y el aislamiento de las operaciones de red.

\subsubsection{Subsistema de Presentación e Interacción}
Representa la superficie de control de cara al usuario. Su objetivo es unificar múltiples canales de administración remota de forma visual y reactiva (ver Figura~\ref{fig:subsistema_presentacion}). Este subsistema se compone de módulos funcionales interdependientes que operan bajo un gestor de estado centralizado:
\begin{itemize}
...
\begin{figure}[ht]
    \centering
    % [PENDIENTE DE EXPORTAR] \includegraphics[width=\linewidth]{images/subsistema_presentacion.png}
    \caption{Diagrama estructural del Subsistema de Presentación e Interacción.}
    \label{fig:subsistema_presentacion}
\end{figure}

\subsubsection{Subsistema de Procesamiento y Control (Core)}
Actúa como el motor de red y de sesión en el que se delegan las operaciones críticas. Como se indica en la Figura~\ref{fig:subsistema_nucleo}, este subsistema administra los algoritmos criptográficos, protocolos robustos de transporte y la auditoría de uso:
\begin{itemize}
...
\begin{figure}[ht]
    \centering
    % [PENDIENTE DE EXPORTAR] \includegraphics[width=\linewidth]{images/subsistema_nucleo.png}
    \caption{Módulos transaccionales y de conectividad del Subsistema de Procesamiento Interno.}
    \label{fig:subsistema_nucleo}
\end{figure}

\subsubsection{Subsistema de Inteligencia Artificial Contextual}
Este subsistema, ilustrado en la Figura~\ref{fig:subsistema_agente}, funge como puente middleware entre la estación cliente local y la red de transformadores y LLMs proveedores de respuestas, habilitando las capacidades de andamiaje:
...
\begin{figure}[ht]
    \centering
    % [PENDIENTE DE EXPORTAR] \includegraphics[width=\linewidth]{images/subsistema_agente.png}
