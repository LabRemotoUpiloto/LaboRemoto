...
Además, cuenta con módulos específicos para la enseñanza y el control de hardware, permitiendo interactuar visualmente con los pines GPIO de una Raspberry Pi.
...
*   **Configuración de Pines**: Cambia modos (Input/Output) y resistencias pull-up/down con un clic.
...
*   **Auditoría y Logs**: Grabación de sesiones de terminal y almacenamiento de logs en la nube o localmente.
*   **Seguridad**: Credenciales locales protegidas con algoritmos de cifrado robustos (`Argon2`, `ChaCha20Poly1305`).

![Formulario de Conexión](apps/desktop/web/public/Formulario.png)

## Cómo Funciona

La aplicación sigue una arquitectura híbrida robusta:

1.  **Frontend (React + TypeScript)**: Proporciona la interfaz de usuario (UI), gestionando el estado de la aplicación, los componentes visuales (terminal, explorador de archivos, chat) y la interacción con el usuario.
2.  **Backend (Rust + Tauri)**: Actúa como el núcleo lógico. Maneja las conexiones de red pesadas, el cifrado, el sistema de archivos y la lógica de negocio crítica.
...
*   **HTTP/HTTPS**: Para la comunicación con servicios de IA y almacenamiento en la nube.
*   **Cifrado Avanzado**:
